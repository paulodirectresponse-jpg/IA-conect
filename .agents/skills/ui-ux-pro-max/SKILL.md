...
If `python` is not found, try `python3`, then `py -3`. Requires Python 3.x and no external dependencies. In IA Connect, resolve the script from the project root at `.agents/skills/ui-ux-pro-max/scripts/search.py`.
