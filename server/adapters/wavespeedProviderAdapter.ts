...
export class WaveSpeedProviderAdapter implements VideoProviderAdapter {
  ...
  private modelName(modelId:string,mode:GenerationMode,providerModelIdentifier?:string,capabilityId?:string){
    ...
    const family=providerModelIdentifier||VIDEO_FAMILIES[modelId],suffix=videoSuffix(mode,capabilityId);
  ...
  private async submitSingle(params:ProviderGenerationParams){
    const model=this.modelName(params.model_id,params.mode,params.provider_model_identifier,String(params.capability_id||'')),controller=new AbortController(),timer=setTimeout(()=>controller.abort(),45000);
    try{
