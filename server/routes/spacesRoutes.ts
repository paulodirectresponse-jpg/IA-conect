...
spacesRouter.get('/spaces/flows/:flowId/runs',async(req:AuthenticatedRequest,res)=>{try{const limit=Math.min(100,Math.max(1,Number(req.query.limit)||50));const runs=await betaFlowEconomicRuntimeService.listPublic(req.user!.uid,100);return res.json({success:true,data:runs.filter(run=>run.flow_id===req.params.flowId).slice(0,limit)});}catch(error){return failure(res,error,'Não foi possível carregar o histórico do Space.');}});
