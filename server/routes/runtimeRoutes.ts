...
function classify(mime: string, filename: string): AssetType {
  ...
  if (
    ...
    ASSET_UPLOAD_LIMITS.VIDEO.allowed_extensions.includes(ext)
  ) {
    return 'VIDEO';
  }
  ...
  if (
    ...
    ASSET_UPLOAD_LIMITS.AUDIO.allowed_extensions.includes(ext)
  ) {
    return 'AUDIO';
  }
    ...
    try {
      ...
      if (size > ASSET_UPLOAD_LIMITS[type].max_bytes) {
        return res.status(413).json({
          ...
          error: {
            code: 'FILE_TOO_LARGE',
            message: 'O arquivo excede o limite máximo permitido de 50 MB.',
          },
        });
      }
      ...
      const bucket = String(
        process.env.SUPABASE_BUCKET || 'ia-conect-assets'
      ).trim();
      ...
      if (!supabaseUrl || !secretKey || !bucket) {
        return res.status(503).json({
          ...
          error: {
            ...
            message: 'Supabase Storage não está configurado no servidor.',
          },
        });
      }

      const assetId = `ast_${Date.now()}_${crypto
        .randomBytes(4)
        .toString('hex')}`;
      ...
      const storagePath =
        `users/${uid}/assets/${assetId}/original${ext}`;
      ...
      try {
        signedData = JSON.parse(responseText);
      } catch {
        signedData = {};
      }
        ...
        return res.status(502).json({
          ...
          error: {
            ...
            message:
              signedData?.message ||
              signedData?.error ||
              `Supabase respondeu HTTP ${signResponse.status}.`,
          },
        });
      }
      ...
      if (!relativeUrl) {
        return res.status(502).json({
          ...
          error: {
            ...
            message:
              'O Supabase não retornou uma URL válida para o upload.',
          },
        });
      }
...
runtimeRouter.post(
  '/workspace/validate-and-preview',
  requireAuth,
  jsonBody,
  async (req: AuthenticatedRequest, res) => {
    try {
      ...
      try {
        quote = await smartRouterService.selectProvider({
          userId: uid,
        ...
        if (e?.code !== 'NO_PROVIDER_AVAILABLE') {
          throw e;
        }
      }
      ...
      const estimated =
        quote?.selected?.customer_price_cents ?? null;
      ...
      const requestDraft = {
        ...
        prompt_compiler_version:
          compiled.prompt_compiler_version,
        ...
        customer_balance_available_cents:
          wallet.available_balance_cents,

        balance_after_generation_cents:
          estimated == null
            ? wallet.available_balance_cents
            : wallet.available_balance_cents - estimated,
      ...
      return res.json({
        ...
        data: {
          ...
          notice:
            estimated != null
              ? 'Preço calculado pela rota configurada mais econômica. Nenhum saldo foi debitado ainda.'
              : 'Nenhum provider configurado e precificado está disponível para esta combinação.',
        },
      });
    } catch (err: any) {
      return res.status(400).json({
