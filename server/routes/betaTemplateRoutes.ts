...
async function requireTemplates(_req:AuthenticatedRequest,res:Response,next:NextFunction){try{const flags=await featureFlagService.getPublicFlags();if(!flags['beta.enabled'])return failure(res,{code:'BETA_DISABLED'},'Beta indisponível.');if(!flags['beta.templates'])return failure(res,{code:'TEMPLATES_DISABLED'},'Templates indisponíveis.');next();}catch{return failure(res,{code:'BETA_ACCESS_UNAVAILABLE'},'Não foi possível validar o acesso.');}}
