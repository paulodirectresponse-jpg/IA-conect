...
export interface BillingControl{
  ...
  legacy_wallet_write_enabled:boolean;
  updated_at:string;
  updated_by?:string;
}
const DOC='app_config/billing_control';
const DEFAULTS:BillingControl={credit_v2_enabled:true,new_generations_enabled:true,provider_execution_enabled:true,yellow_execution_enabled:false,legacy_wallet_write_enabled:false,updated_at:new Date(0).toISOString()};
let cache:BillingControl|null=null,cacheAt=0;const TTL=5000;
function normalize(raw:any={}):BillingControl{return{credit_v2_enabled:raw.credit_v2_enabled!==false,new_generations_enabled:raw.new_generations_enabled!==false,provider_execution_enabled:raw.provider_execution_enabled!==false,yellow_execution_enabled:Boolean(raw.yellow_execution_enabled),legacy_wallet_write_enabled:Boolean(raw.legacy_wallet_write_enabled),updated_at:String(raw.updated_at||new Date(0).toISOString()),updated_by:raw.updated_by};}
