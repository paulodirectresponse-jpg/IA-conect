...
export const creditPricingService={
 async preview(input:CreditPricingInput){
  const signature=pricingSignatureService.create({model_id:input.model_id,mode:input.mode,resolution:input.resolution,duration_seconds:input.duration_seconds,aspect_ratio:input.aspect_ratio,number_of_outputs:input.number_of_outputs,audio_enabled:input.audio_enabled,reference_mode:referenceMode(input.references),reference_count:(input.references||[]).length,model_variant:input.model_variant,pricing_options:input.pricing_options});
  const preliminary=await smartRouterService.selectProvider({...input,force_live_quote:input.force_live_quote});
