...
export interface PricingHealthRow {key:string;provider_id:string;provider_name:string;provider_model_identifier?:string;model_id:string;capability_id?:string;mode:GenerationMode;duration_seconds:number;resolution:string;provider_cost_usd:number|null;provider_cost_brl_cents:number|null;safe_cost_brl_cents:number|null;fully_loaded_safe_cogs_cents?:number|null;retail_credit_price?:number|null;retail_pricing_version?:number|null;margin_percent:number|null;status:'OK'|'FAILED';checked_at:string;error?:string;pricing_signature_hash?:string;health?:'GREEN'|'YELLOW'|'RED'|'PROBE';pricing_unit?:string;stale?:boolean;source_checked_at?:string;}
...
function routeIdentity(row:Pick<PricingHealthRow,'provider_id'|'provider_model_identifier'|'model_id'|'capability_id'|'mode'|'duration_seconds'|'resolution'>){
 return [row.provider_id,row.provider_model_identifier||'',row.model_id,row.capability_id||'',row.mode,Number(row.duration_seconds||1),String(row.resolution||'').toLowerCase()].join('|');
}
function recoverRecentGoodRows(current:PricingHealthRow[],previous:PricingHealthSnapshot){
 const MAX_STALE_MS=2*60*60*1000,now=Date.now(),good=new Map<string,PricingHealthRow>();
 for(const row of previous.rows||[]){const age=now-Date.parse(String(row.checked_at||0));if(row.status==='OK'&&Number.isFinite(age)&&age>=0&&age<=MAX_STALE_MS)good.set(routeIdentity(row),row);}
 return current.map(row=>{if(row.status==='OK')return row;const prior=good.get(routeIdentity(row));if(!prior)return row;return{...prior,key:row.key,stale:true,source_checked_at:prior.checked_at,checked_at:row.checked_at,error:row.error?('Falha atual: '+row.error+' · usando último preço válido por até 2h.'): 'Usando último preço válido por até 2h.'};});
}
...
export const pricingSyncService={
 ...
 async runHourlySync(){
  const checkedAt=new Date().toISOString();const previous=await pricingSyncService.getLatestSnapshot();quoteCacheService.invalidateAll();
  ...
  const resilientRows=recoverRecentGoodRows(rows,previous);
  latestSnapshot={checked_at:checkedAt,checked:resilientRows.length,healthy:resilientRows.filter(r=>r.status==='OK'&&r.health!=='RED').length,failed:resilientRows.filter(r=>r.status==='FAILED'||r.health==='RED').length,fx_rate:fx.rate,fx_source:fx.source,cache_ttl_minutes:Math.round(quoteCacheService.ttl_ms()/60000),provider_finance:providerFinance,rows:resilientRows,signatures_checked:new Set(resilientRows.map(r=>r.pricing_signature_hash).filter(Boolean)).size,normal_floor_margin_percent:settings.normal_floor_margin_percent,emergency_floor_margin_percent:settings.emergency_floor_margin_percent,conservative_credit_value_micros:settings.conservative_credit_value_micros,route_refresh:routeRefresh};
