...
export const assetService = {
  ...
  async reserveUpload(params: {
    ...
    return assetRepository.createAsset({
      ...
      category,
      name: params.name || params.filename || 'Novo arquivo',
      alias: params.alias,
  ...
  async completeUpload(params: {
    ...
    const existing = await assetRepository.getAsset(params.assetId, params.userId);
    if (!existing) throw new Error('Arquivo não encontrado ou sem permissão.');
    if (existing.status === 'READY') return existing;
    if (existing.status !== 'UPLOADING') throw new Error('O arquivo não está em estado de envio.');

    return assetRepository.updateAsset(params.assetId, params.userId, {
      status: 'READY',
      width: Number.isFinite(params.width) ? Number(params.width) : existing.width ?? null,
      height: Number.isFinite(params.height) ? Number(params.height) : existing.height ?? null,
      duration_seconds: Number.isFinite(params.duration_seconds) ? Number(params.duration_seconds) : existing.duration_seconds ?? null,
    });
  },
