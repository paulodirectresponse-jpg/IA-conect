...
export const paymentService={
 ...
 async processWebhook(params:{headers:Record<string,any>;dataId:string;eventType?:string;action?:string}){
  ...
  // The Mercado Pago simulator may send an Order sample even when the legacy
  // Payments topic is selected. Acknowledge authenticated, unsupported events
  // without trying to look an order ID up in /v1/payments.
  const eventType=String(params.eventType||'').toLowerCase();
  if(eventType && eventType!=='payment')return{ignored:true,reason:`unsupported_event:${eventType}`};
