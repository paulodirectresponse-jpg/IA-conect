...
async function snapshot(userId:string,type:BetaShareResourceType,id:string){
  ...
  const a:any=await workflowAppService.get(userId,id);const inputSchema=(a.input_schema||[]).filter((f:any)=>f.exposed).map((f:any)=>({node_id:f.node_id,label:f.label,media_type:f.media_type,required:f.required,placeholder:f.placeholder,help_text:f.help_text,order:f.order}));const outputSchema=(a.output_schema||[]).filter((f:any)=>f.exposed).map((f:any)=>({node_id:f.node_id,label:f.label,media_type:f.media_type,order:f.order}));return{title:a.name,data:{app_id:a.app_id,name:a.name,description:a.description,status:a.status,revision:a.revision,source_type:a.source_type,input_schema:inputSchema,output_schema:outputSchema,published_at:a.published_at||null,created_at:a.created_at,updated_at:a.updated_at}};
}
