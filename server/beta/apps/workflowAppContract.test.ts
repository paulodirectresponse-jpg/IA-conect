...
describe('PR-16 Workflow Apps contracts',()=>{
 ...
 it('supports friendly public input and output schemas',()=>{const types=read('server/beta/apps/workflowAppTypes.ts');for(const token of ['label:string','exposed:boolean','required:boolean','placeholder:string','help_text:string','order:number','default_value:any'])expect(types).toContain(token);expect(types).toContain('WorkflowAppOutputField');});
 it('maps only configured inputs and exposed outputs at runtime',()=>{const service=read('server/beta/apps/workflowAppService.ts');expect(service).toContain('for(const field of app.input_schema)');expect(service).toContain('field.exposed?');expect(service).toContain('app.output_schema.filter(item=>item.exposed)');expect(service).toContain('visibleOutputs');});
