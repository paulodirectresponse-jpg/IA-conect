...
function fail(message:string):never{throw Object.assign(new Error(message),{code:'VALIDATION_ERROR'});}
...
export const betaTemplateService={
  ...
  async get(userId:string,templateId:string){const row=await betaTemplateRepository.get(templateId,userId);if(!row)fail('Template não encontrado.');return row;},
  async createFromFlow(userId:string,input:any){
    ...
    if(!graph.nodes.length)fail('Salve um Flow com nós antes de criar um template.');
