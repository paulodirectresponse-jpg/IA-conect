...
export const assetRepository = {
  ...
  async createAsset(params: { owner_user_id:string; type:AssetType; category:AssetCategory; name:string; alias?:string; storage_path:string; public_url?:string; thumbnail_url?:string; mime_type:string; size_bytes:number; width?:number|null; height?:number|null; duration_seconds?:number|null; status?:AssetStatus }): Promise<Asset> {
    const assetId = `ast_${Date.now()}_${crypto.randomBytes(4).toString('hex')}`;
    let base = sanitizeAlias(params.alias || params.name) || `asset_${Date.now().toString().slice(-6)}`;
    let alias = base; let i = 1;
    while (await this.findByAlias(alias, params.owner_user_id)) alias = `${base}_${i++}`;
    const now = new Date().toISOString();
    const asset: Asset = { asset_id:assetId, owner_user_id:params.owner_user_id, type:params.type, category:params.category, name:params.name.trim(), alias, storage_path:params.storage_path, public_url:params.public_url || '', thumbnail_url:params.thumbnail_url || params.public_url || '', mime_type:params.mime_type, size_bytes:params.size_bytes, width:params.width ?? null, height:params.height ?? null, duration_seconds:params.duration_seconds ?? null, status:params.status || 'READY', created_at:now, updated_at:now, deleted_at:null };
  ...
  async updateAsset(assetId:string,userId:string,updates:{name?:string;alias?:string;category?:AssetCategory;status?:AssetStatus;public_url?:string}): Promise<Asset> {
    const existing = await this.getAsset(assetId,userId); if(!existing) throw new Error('Asset não encontrado ou sem permissão.');
    if (updates.alias && updates.alias !== existing.alias) {
      const clean = sanitizeAlias(updates.alias); if(!clean) throw new Error('Alias inválido.');
      const collision = await this.findByAlias(clean,userId); if(collision && collision.asset_id!==assetId) throw new Error(`O alias @${clean} já está em uso.`);
      updates.alias = clean;
    }
  ...
  async softDeleteAsset(assetId:string,userId:string): Promise<boolean> {
    const existing=await this.getAsset(assetId,userId); if(!existing) throw new Error('Asset não encontrado ou sem permissão.');
    const now=new Date().toISOString(); await dbOrThrow().collection('assets').doc(assetId).update({deleted_at:now,updated_at:now}); return true;
  }
};
