...
const ImageEditorView=lazy(()=>import('./components/views/ImageEditorView.js').then(m=>({default:m.ImageEditorView})));
const VideoEditorView=lazy(()=>import('./components/views/VideoEditorView.js').then(m=>({default:m.VideoEditorView})));
const SpacesView=lazy(()=>import('./components/views/SpacesView.js').then(m=>({default:m.SpacesView})));
const AiConversationView=lazy(()=>import('./components/views/AiConversationView.js').then(m=>({default:m.AiConversationView})));
...
const MainApp: React.FC<{onSignedOut?:()=>void}> = ({onSignedOut}) => {
  ...
  useEffect(()=>{
    if(loading||!firebaseUser||typeof window==='undefined')return;
    const params=new URLSearchParams(window.location.search);
    if(params.get('subscription')==='return'){
      setLastStableView('wallet');
  ...
  return <AppLayout currentView={currentSafeView} onNavigate={navigate} betaEnabled={betaEnabled}><Suspense fallback={<ViewFallback/>}>{currentSafeView==='dashboard'&&<DashboardView onNavigate={navigate}/>} {currentSafeView==='wallet'&&<WalletView onNavigate={navigate}/>}{currentSafeView==='create-video'&&<CreateHubView initialMode="VIDEO"/>}{currentSafeView==='create-image'&&<CreateHubView initialMode="IMAGE"/>}{currentSafeView==='create-voice'&&<VoiceCreateView/>}{currentSafeView==='create-music'&&<MusicCreateView/>}{currentSafeView==='create-3d'&&<ThreeDCreateView/>}{currentSafeView==='edit-image'&&<ImageEditorView/>}{currentSafeView==='edit-video'&&<VideoEditorView/>}{currentSafeView==='spaces'&&<SpacesView/>}{currentSafeView==='ai'&&<AiConversationView/>}{currentSafeView==='community'&&<CommunityView onNavigate={navigate}/>} {currentSafeView==='history'&&<HistoryView/>}{currentSafeView==='library'&&<LibraryHubView/>}{currentSafeView==='admin'&&isAdmin&&<AdminView/>}{currentSafeView==='settings'&&<SettingsView/>}</Suspense></AppLayout>;
};
