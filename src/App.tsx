...
const MainApp: React.FC<{onSignedOut?:()=>void}> = ({onSignedOut}) => {
  ...
  const navigate = (view:string) => {
    if(view==='beta'&&!betaEnabled)return;
    markViewNavigationStart(view === 'assets' ? 'library' : view);
    if(view!=='beta')setLastStableView(view);
    setActiveView(view);
  };
  ...
  useEffect(()=>{
    ...
    let active=true;
    getBetaEnabled().then(enabled=>{if(active)setBetaEnabled(enabled)}).catch(()=>{if(active)setBetaEnabled(false)});
    return()=>{active=false};
  },[loading,firebaseUser]);
  useEffect(()=>{
    if(activeView==='beta'&&!betaEnabled)setActiveView(lastStableView);
  },[activeView,betaEnabled,lastStableView]);
  ...
  if(activeView==='beta'&&betaEnabled){
    return <Suspense fallback={<BetaFallback/>}><BetaApp onExit={()=>navigate(lastStableView)}/></Suspense>;
  }
  ...
  return <AppLayout currentView={currentSafeView} onNavigate={navigate} betaEnabled={betaEnabled}>
...
export default function App({onSignedOut}:{onSignedOut?:()=>void}={}){ return <AuthProvider><MainApp onSignedOut={onSignedOut}/></AuthProvider>; }