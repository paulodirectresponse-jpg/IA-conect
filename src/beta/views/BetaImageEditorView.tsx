...
export const BetaImageEditorView:React.FC<{initialAssetId?:string|null;onOpenLibrary?:()=>void}>=({initialAssetId,onOpenLibrary})=>{
 ...
 const buildRequest=async()=>{
  ...
  return{capability_id:tool,model_id:model.model_id,prompt:prompt.trim(),references:refs,controls:{resolution,aspect_ratio:aspectRatio,output_format:'png',number_of_outputs:1,editor_operation:operation[tool],background_mode:tool==='background-remove-replace'?backgroundMode:undefined,variation_strength:tool==='variations'?variationStrength:undefined}};
 };
