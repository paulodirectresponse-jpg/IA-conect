...
export const BetaHomeView: React.FC<{pendingIntent?:BetaLibraryIntent|null;onOpenLibrary?:()=>void;audioEnabled?:boolean;onOpenAudio?:()=>void;imageEditorEnabled?:boolean;onOpenImageEditor?:()=>void;videoEnabled?:boolean;onOpenVideo?:()=>void;threeDEnabled?:boolean;onOpenThreeD?:()=>void}> = ({pendingIntent,onOpenLibrary,audioEnabled,onOpenAudio,imageEditorEnabled,onOpenImageEditor,videoEnabled,onOpenVideo,threeDEnabled,onOpenThreeD}) => (
    ...
    <section className="ia-beta-section" aria-labelledby="ia-beta-modules-title">
      ...
      <div className="ia-beta-module-grid">
        {modules.map(({ key, title, description, icon: Icon }) => (
          <article className={`ia-beta-module-card ${(key==='image'&&imageEditorEnabled)||(key==='video'&&videoEnabled)||(key==='audio'&&audioEnabled)||(key==='3d'&&threeDEnabled)?'is-available':''}`} key={key}>
