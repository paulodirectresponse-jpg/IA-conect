...
export const generationClient = {
  async create(draft: GenerationRequestDraft): Promise<Generation> {
    return apiRequest<Generation>('/api/generations', {
      ...
      body: JSON.stringify({
        ...
        requested_provider_id: (draft as any).provider_id,
  ...
  async list(max = 50): Promise<Generation[]> { return apiRequest<Generation[]>(`/api/generations?limit=${Math.min(100, max)}`); },
