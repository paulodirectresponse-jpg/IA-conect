...
export interface AiConversationContext{conversation_id:string;summary:string;creative_state:Record<string,any>;references:Array<{label:string;kind:string;value:string;message_id:string|null;asset_id:string|null;asset_type?:'IMAGE'|'VIDEO'|'AUDIO'|'MODEL_3D'|null}>;last_intent:AiConversationIntent;readiness:AiIntentReadiness;missing_information:string[];revision:number;updated_at:string;}
