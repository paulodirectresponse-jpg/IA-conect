...
export interface ProviderModelMatchProposal {provider_id:string;model_id:string;model_name:string;function_id:CuratedModelFunction;capability_id:string;provider_model_identifier:string;candidate_name:string;confidence:number;match_reason:string;already_mapped:boolean;}
export interface ProviderScanResult {scan_id:string;provider_id:string;provider_name:string;configured:boolean;discovery_mode:'CATALOG_API'|'SEARCH_API'|'CURATED_REQUIRED';candidate_count:number;candidate_sample_count:number;candidates:ProviderScanCandidate[];matched_count:number;matches:ProviderModelMatchProposal[];warning?:string|null;error?:string|null;scanned_at:string;}
...
export const adminService = {
  ...
  async getProviderScans(){return apiRequest<{latest:ProviderScanResult[];pricing:ProviderPricingAdmin[];mappings:ProviderModelMapping[]}>('/api/admin/provider-scan');},
  async getProviderScanInventory(){return apiRequest<CuratedModelInventory>('/api/admin/provider-scan/inventory');},
  async scanProviders(provider_id?:string){return apiRequest<ProviderScanResult|ProviderScanResult[]>('/api/admin/provider-scan',{method:'POST',body:JSON.stringify(provider_id?{provider_id}:{})});},
  async saveProviderPricing(data:Partial<ProviderPricingAdmin>&Pick<ProviderPricingAdmin,'provider_id'|'provider_model_identifier'|'unit'|'unit_price_usd'>){return apiRequest<ProviderPricingAdmin>('/api/admin/provider-pricing',{method:'POST',body:JSON.stringify(data)});},
  async approveProviderMapping(data:{provider_id:string;model_id:string;provider_model_identifier:string;capability_id:string}){return apiRequest<{mapping:ProviderModelMapping;pricing:ProviderPricingAdmin;model_status:string;beta_only:boolean}>('/api/admin/provider-scan/approve-mapping',{method:'POST',body:JSON.stringify(data)});},
