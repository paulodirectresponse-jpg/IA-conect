  ...
  return (
    <div className="space-y-2">
      ...
      <div className="relative rounded-[14px] border border-white/[0.075] bg-[#0a0d12] focus-within:border-cyan-400/25 transition-colors overflow-visible">
        ...
        {mentionOpen && <div className="absolute z-[100] left-2 right-2 top-full mt-1.5 overflow-hidden rounded-xl border border-white/[0.09] bg-[#11151c] shadow-2xl shadow-black/60"><div className="px-3 py-2 border-b border-white/[0.06] flex items-center justify-between gap-3"><div><p className="text-[9px] font-bold text-white">Referências deste job</p><p className="text-[8px] text-zinc-600">Selecione para inserir no prompt.</p></div><button type="button" onClick={() => { setMentionOpen(false); onRequestAddMedia(); }} className="text-[8px] font-semibold text-cyan-300 flex items-center gap-1"><Plus className="w-3 h-3"/> Adicionar</button></div><div className="max-h-60 overflow-y-auto p-1.5">{suggestions.map((ref, index) => { const asset = ref.asset; const imageUrl = asset?.thumbnail_url || asset?.public_url; return <button key={ref.asset_id} type="button" onMouseEnter={() => setActiveIndex(index)} onClick={() => insertReference(ref)} className={`w-full flex items-center gap-2 p-2 rounded-lg text-left transition-colors ${index === activeIndex ? 'bg-white/[0.07]' : 'hover:bg-white/[0.04]'}`}><div className="w-10 h-10 rounded-lg bg-[#0b0e13] overflow-hidden shrink-0 flex items-center justify-center text-zinc-500 border border-white/[0.06]">{asset?.type === 'IMAGE' && imageUrl ? <img src={imageUrl} className="w-full h-full object-cover" alt=""/> : mediaIcon(asset?.type)}</div><div className="min-w-0 flex-1"><p className="text-[9px] font-semibold text-white truncate">{asset?.name || ref.alias_snapshot}</p><div className="flex items-center gap-2 mt-0.5"><span className="text-[8px] font-mono font-bold text-cyan-300">@{ref.alias_snapshot}</span><span className="text-[7px] uppercase text-zinc-700">{asset?.type || 'IMAGE'}</span></div></div></button>; })}{!suggestions.length && <div className="p-4 text-center"><p className="text-[9px] text-zinc-600">{promptReferences.length ? 'Nenhuma referência corresponde à busca.' : 'Nenhuma mídia anexada ainda.'}</p><button type="button" onClick={() => { setMentionOpen(false); onRequestAddMedia(); }} className="mt-2 text-[9px] font-semibold text-cyan-300">+ Adicionar referência</button></div>}</div></div>}
      </div>
