...
export const PromptComposer: React.FC<PromptComposerProps> = ({
  ...
  const highlightedPrompt=useMemo(()=>{
    ...
    return prompt.split(regex).map((part,index)=>active.has(part.toLowerCase())?<span key={`${part}-${index}`} className="rounded-[5px] border border-cyan-300/45 bg-cyan-300/15 px-[2px] py-[1px] font-bold text-cyan-200 box-decoration-clone">{part}</span>:<React.Fragment key={`text-${index}`}>{part}</React.Fragment>);
  ...
  return <div className="space-y-2">
    ...
    <div className="relative rounded-[14px] border border-white/[0.075] bg-[#0a0d12] focus-within:border-cyan-400/25 transition-colors overflow-visible">
      <div ref={highlightRef} aria-hidden="true" className="absolute left-0 right-0 top-0 bottom-10 p-3 text-[11px] font-medium leading-relaxed whitespace-pre-wrap break-words overflow-hidden pointer-events-none text-white">{highlightedPrompt}{prompt.endsWith('\n')?'\u200b':null}</div>
