...
export const SHOWCASE_MODELS:ShowcaseItem[]=[
  {modelId:'kling-3.0',name:'Kling 3.0',mode:'VIDEO',videoSrc:'/media/showcase/kling-3.mp4',sourceLabel:'IA Connect showcase',headline:'Personagens, acting e movimento cinematográfico',description:'Movimento natural, direção de câmera e takes com acabamento cinematográfico.',badges:['cinematográfico','personagens','vídeo'],availableInStudio:true},
  {modelId:'gemini-omni-flash',name:'Omni Flash',mode:'VIDEO',videoSrc:'/media/showcase/omni-flash.mp4',sourceLabel:'IA Connect showcase',headline:'Vídeo multimodal com velocidade e qualidade',description:'Transforme texto, imagem e referências em um fluxo de criação multimodal.',badges:['multimodal','áudio','vídeo'],availableInStudio:true},
  {modelId:'seedance-2.5',name:'Seedance 2.5',mode:'VIDEO',videoSrc:'/media/showcase/seedance-2-5.mp4',sourceLabel:'IA Connect showcase',headline:'Cenas longas e consistência visual',description:'Demonstração selecionada para comparar linguagem, ritmo e continuidade.',badges:['consistência','cenas','vídeo'],availableInStudio:false},
  {modelId:'wan-3.0',name:'WAN 3.0',mode:'VIDEO',videoSrc:'/media/showcase/wan-3.mp4',sourceLabel:'IA Connect showcase',headline:'Realismo cinematográfico e referências',description:'Exemplo de geração para comparar textura, composição e movimento.',badges:['realismo','referências','vídeo'],availableInStudio:false},
  {modelId:'wan-3.0-prime',name:'WAN 3.0 Prime',mode:'VIDEO',videoSrc:'/media/showcase/wan-3-prime.mp4',sourceLabel:'IA Connect showcase',headline:'Uma variação premium para comparação',description:'Um segundo resultado do ecossistema WAN para ampliar a descoberta visual.',badges:['premium','comparação','vídeo'],availableInStudio:false},
];
...
export const ModelShowcase:React.FC<{compact?:boolean;onTry?:(item:ShowcaseItem)=>void;title?:string;subtitle?:string}> = ({compact=false,onTry,title='Modelos em destaque',subtitle='Veja os resultados em movimento e entre direto no modelo certo.'}) => <section>
  ...
  <div className={`mt-5 grid ${compact?'md:grid-cols-2 xl:grid-cols-5':'md:grid-cols-2 xl:grid-cols-5'} gap-3`}>
    {SHOWCASE_MODELS.map(item=><article key={item.modelId} className="group overflow-hidden rounded-2xl border border-sky-300/[0.1] bg-[#07111b] hover:border-sky-300/[0.24] transition-all hover:-translate-y-0.5">
      <div className="relative aspect-video bg-[#050b12] overflow-hidden">
        <video className="absolute inset-0 w-full h-full object-cover scale-[1.01]" src={item.videoSrc} autoPlay muted loop playsInline preload="metadata" disablePictureInPicture/>
