...
export const AssetPickerModal: React.FC<AssetPickerModalProps> = ({
  ...
  return (
    <ResponsiveDialogShell open={isOpen} onClose={onClose} ariaLabel={title} mobileMode="fullscreen" desktopMaxWidth={980} backdropClassName="ia-asset-picker-backdrop" className="ia-asset-picker-shell flex h-[min(700px,90vh)] flex-col overflow-hidden bg-[#0b0e13] text-zinc-100">
        ...
        <div className="ia-asset-picker-body flex min-h-0 flex-1">
          <aside className="ia-asset-picker-sidebar w-[210px] shrink-0 overflow-y-auto border-r border-white/[0.06] bg-[#090c11] p-3">
            <p className="mb-2 px-2 text-[8px] font-bold uppercase tracking-[0.18em] text-zinc-700">Biblioteca</p>
            <button type="button" onClick={() => { setActiveTab('LIBRARY'); setScope('GLOBAL'); setActiveProjectId(''); }} className={`ia-asset-picker-project-chip flex h-9 w-full items-center gap-2 rounded-xl px-2.5 text-[10px] font-semibold ${scope==='GLOBAL'?'is-active text-white':'text-zinc-300 hover:bg-white/[0.04]'}`}><Library className="h-3.5 w-3.5" /> Geral</button>
            <div className="ia-asset-picker-projects mt-4 space-y-1">
              {projects.map((project) => (
                <button key={project.entity_id} type="button" onClick={() => { setActiveTab('LIBRARY'); setScope('PROJECT'); setActiveProjectId(project.entity_id); }} className={`ia-asset-picker-project-chip flex min-h-9 w-full items-center gap-2 rounded-xl px-2.5 py-2 text-left text-[9px] ${scope==='PROJECT'&&activeProjectId===project.entity_id?'is-active text-white':'text-zinc-500 hover:bg-white/[0.04] hover:text-white'}`}><FolderKanban className="h-3.5 w-3.5" /><span className="truncate">{project.name}</span></button>
              ))}
            </div>
          ...
          <main className="ia-asset-picker-main min-w-0 flex-1 overflow-y-auto p-4 sm:p-5">
            {activeTab === 'UPLOAD' ? (
              <div className="flex h-full flex-col">
                ...
                <div
                  ...
                  className="ia-asset-picker-upload-zone flex min-h-[300px] flex-1 cursor-pointer flex-col items-center justify-center rounded-2xl border border-dashed border-white/[0.09] bg-white/[0.02] px-8 text-center"
                >
                  {uploading ? <><Loader2 className="h-6 w-6 animate-spin text-cyan-300" /><p className="mt-3 text-xs">{activeFileName}</p><p className="mt-1 text-[9px] text-zinc-600">{uploadProgress}%</p></> : <><Upload className="h-7 w-7 text-zinc-600" /><p className="mt-4 text-sm font-semibold"><span className="ia-desktop-copy">Clique ou arraste um arquivo</span><span className="ia-mobile-copy">Adicionar arquivo</span></p><p className="ia-mobile-copy mt-1 text-[9px] text-zinc-600">Fotos, câmera ou arquivos do aparelho</p></>}
                </div>
              ...
              <div className="space-y-4">
                <div className="ia-asset-picker-content-tabs grid grid-cols-2 gap-2 md:grid-cols-4">
