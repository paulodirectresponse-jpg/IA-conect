...
export const CreatorPanel:React.FC<Props>=(p)=>{
 ...
 const frameMode=Boolean(p.initialImage||p.endImage),referenceMode=p.references.length>0;
 ...
 const hasAdvanced=Boolean(caps?.supports_audio_generation||caps?.supports_seed||caps?.supports_motion_strength);
 ...
 const frameCard=(asset:Asset|null,label:string,target:'INITIAL'|'END',enabled:boolean)=>{
  const blocked=referenceMode||!enabled;
 ...
 const referencesCard=<div onDragEnter={e=>{if(canGeneral&&!frameMode){e.preventDefault();setDragTarget('GENERAL')}}} onDragOver={e=>{if(canGeneral&&!frameMode)e.preventDefault()}} onDragLeave={()=>setDragTarget(v=>v==='GENERAL'?null:v)} onDrop={e=>{if(canGeneral&&!frameMode)acceptDrop('GENERAL',e)}} className={`relative min-h-[76px] rounded-xl border p-2 ${canGeneral&&!frameMode?'border-white/[0.07] bg-white/[0.025]':'border-white/[0.04] bg-white/[0.012] opacity-50'} ${dragTarget==='GENERAL'?'border-cyan-300/55 bg-cyan-300/[0.08]':''}`}>
  <button type="button" disabled={!canGeneral||frameMode} onClick={()=>p.onOpenPicker('GENERAL')} className="absolute inset-0 w-full h-full rounded-xl disabled:cursor-not-allowed"/>
  <div className="relative pointer-events-none h-full flex items-center gap-2"><div className="w-9 h-9 rounded-lg border border-white/[0.06] bg-black/20 grid place-items-center shrink-0"><Layers3 className="w-4 h-4 text-zinc-500"/></div><div className="min-w-0"><p className="text-[9px] font-semibold text-zinc-300">Referências</p><p className="text-[7px] text-zinc-700 truncate">{frameMode?'indisponível com frames':acceptedRefs||'não suportado'}</p>{p.references.length>0&&<p className="mt-1 text-[8px] font-bold text-cyan-300">{p.references.length} anexada{p.references.length===1?'':'s'}</p>}</div></div>
 ...
 return <aside className="w-full md:w-[344px] xl:w-[356px] h-full shrink-0 bg-[#090c11] border-r border-white/[0.06] flex flex-col">
  <div className="flex-1 overflow-y-auto px-3 py-3 space-y-2.5">
   ...
   <section>
    <div className="mb-1.5 flex items-center justify-between"><div><p className="text-[9px] font-bold text-zinc-400">Entradas</p><p className="text-[7.5px] text-zinc-700">{modeLabel(p.resolvedMode)}</p></div><span className="text-[7px] text-zinc-700">clique, cole ou arraste</span></div>
