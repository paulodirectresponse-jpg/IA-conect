...
import {
  ChevronDown, Clock3, Image as ImageIcon, Layers3, Plus, Ratio, Settings2,
  SlidersHorizontal, WandSparkles, X, UploadCloud,
} from 'lucide-react';
...
interface Props {
  models: ModelRegistryItem[]; pricing: PricingEntry[]; selectionMode: 'AUTO' | 'MANUAL'; selectedModelId: string;
  ...
  onQuickUpload?: (target: 'INITIAL' | 'END' | 'GENERAL', files: File[]) => void;
  uploadBusy?: boolean;
...
export const CreatorPanel: React.FC<Props> = (p) => {
  ...
  const slot = (asset: Asset | null, label: string, target: 'INITIAL'|'END') => <button type="button" disabled={target==='END' && !caps?.supports_start_end_image} onClick={()=>p.onOpenPicker(target)} onDragEnter={(e)=>{e.preventDefault();setDragTarget(target)}} onDragOver={(e)=>e.preventDefault()} onDragLeave={()=>setDragTarget(v=>v===target?null:v)} onDrop={(e)=>acceptDrop(target,e)} className={`relative h-[68px] rounded-xl overflow-hidden border bg-white/[0.025] disabled:opacity-35 text-left transition-colors ${dragTarget===target?'border-cyan-300/60 bg-cyan-300/[0.08] ring-2 ring-cyan-300/10':'border-white/[0.07] hover:border-white/[0.12]'}`}>
    {asset?.public_url ? <img src={asset.thumbnail_url || asset.public_url} className="w-full h-full object-cover" alt=""/> : <div className="w-full h-full flex flex-col items-center justify-center gap-1 text-zinc-700"><ImageIcon className="w-4 h-4"/><span className="text-[9px]">{dragTarget===target?'Solte a imagem':label}</span><span className="text-[7px] text-zinc-700">clique ou arraste</span></div>}
