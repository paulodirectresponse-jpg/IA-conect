...
export const UnifiedImageCreatorPanel:React.FC<Props>=(p)=>{
 ...
 >
   ...
   <div className="space-y-1.5">
    {ratios.length>0&&settingRow('ratio',Ratio,'Proporção',p.aspectRatio,optionGrid(ratios,p.aspectRatio,p.onChangeAspectRatio))}
    {p.availableResolutions.length>0&&settingRow('resolution',SlidersHorizontal,'Resolução',p.resolution,optionGrid(p.availableResolutions,p.resolution,p.onChangeResolution))}
    {(Number(p.activeModel?.supported_controls?.max_outputs)||0)>1&&settingRow('outputs',Layers3,'Quantidade',String(p.numberOfOutputs),<div>{optionGrid(Array.from({length:Math.min(4,Number(p.activeModel?.supported_controls?.max_outputs)||1)},(_,index)=>index+1),p.numberOfOutputs,p.onChangeNumberOfOutputs)}{p.unitPrice!=null&&<p className="mt-2 text-[8px] text-zinc-600">{formatCredits(p.unitPrice)} por imagem · preço fixo × quantidade</p>}</div>)}
   </div>
