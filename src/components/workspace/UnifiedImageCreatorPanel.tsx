...
import{getImageModelCover}from'../../config/imageModelCovers.js';
...
export const UnifiedImageCreatorPanel:React.FC<Props>=(p)=>{
 ...
 const selectedCoverUrl=p.selectionMode==='MANUAL'?getImageModelCover(p.selectedModelId):null;
 ...
 const settingRow=(id:Exclude<OpenCard,null>,icon:any,label:string,value:string,body:React.ReactNode)=>{const Icon=icon,opened=openCard===id;return <div className={`rounded-xl border transition-colors ${opened?'border-cyan-300/20 bg-cyan-300/[0.035]':'border-white/[0.065] bg-white/[0.025]'}`}><button type="button" onClick={()=>setOpenCard(opened?null:id)} className="h-10 w-full px-3 flex items-center gap-2"><span className="ia-generator-setting-icon"><Icon className="w-3.5 h-3.5"/></span><span className="text-[10px] font-semibold text-zinc-300">{label}</span><span className="ml-auto text-[10px] font-bold text-white">{value}</span><ChevronDown className={`w-3.5 h-3.5 text-zinc-600 transition-transform ${opened?'rotate-180':''}`}/></button>{opened&&<div className="px-2.5 pb-2.5 pt-0.5">{body}</div>}</div>};
 return <aside className="ia-generator-panel ia-generator-panel-image w-full md:w-[352px] xl:w-[368px] h-full shrink-0 flex flex-col">
  <div className="ia-generator-scroll flex-1 overflow-y-auto px-3.5 py-3.5 space-y-3">
  ...
  <div className="ia-generator-actionbar shrink-0 p-3.5">{p.error&&<div role="alert" aria-live="assertive" className="mb-2 rounded-xl border border-rose-400/15 bg-rose-500/[0.08] px-3 py-2 text-[9px] leading-relaxed text-rose-300">{p.error}</div>}{p.generating&&!p.error&&<div aria-live="polite" className="mb-2 rounded-xl border border-cyan-300/10 bg-cyan-300/[0.05] px-3 py-2 text-[9px] text-cyan-200">Geração enviada. Aguardando o processamento...</div>}<div className="mb-2 flex items-center justify-between"><div><p className="text-[8px] uppercase tracking-wider text-zinc-700">Preço</p><p className="text-[11px] font-bold text-white">{p.totalPrice==null?'Preço indisponível':formatCredits(p.totalPrice)}</p></div><div className="text-right"><p className="text-[8px] text-zinc-700">Saldo</p><p className={`text-[10px] font-semibold ${p.hasBalance?'text-zinc-400':'text-rose-400'}`}>{formatCredits(p.balance)}</p></div></div><button disabled={p.generating||p.priceLoading||!p.prompt.trim()||p.totalPrice==null||!p.hasBalance} onClick={p.onGenerate} className="ia-generator-generate w-full h-11.5 rounded-[12px] text-[12px] font-black flex items-center justify-center gap-2 disabled:opacity-35 disabled:grayscale"><Sparkles className="w-4 h-4"/>{p.generating?'Gerando...':p.totalPrice==null?'Gerar imagem':`Gerar imagem • ${formatCredits(p.totalPrice)}`}</button></div>
 </aside>;
};
