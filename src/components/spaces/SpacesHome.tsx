...
export const SpacesHome:React.FC<Props>=({flows,homeItems,loading,creating,deletingId,error,onCreate,onOpen,onDelete})=><div className="h-full overflow-y-auto bg-[#090d12] px-5 py-7 text-zinc-100 sm:px-7 lg:px-9 xl:px-10">
 <div className="mx-auto w-full max-w-[1500px]">
  ...
  <div className="mt-8">
   ...
   {loading?<div className="grid min-h-[320px] place-items-center rounded-2xl border border-white/[0.05] bg-white/[0.015]"><div className="text-center"><LoaderCircle className="mx-auto h-5 w-5 animate-spin text-cyan-300"/><p className="mt-2 text-[10px] text-zinc-500">Carregando Spaces…</p></div></div>:flows.length?<div className="grid grid-cols-1 gap-4 sm:grid-cols-2 xl:grid-cols-3 2xl:grid-cols-4">{flows.map(flow=>{const item=homeItems.find(row=>row.flow.flow_id===flow.flow_id);return <article key={flow.flow_id} className="group relative aspect-[4/3] overflow-hidden rounded-2xl border border-white/[0.065] bg-[#11161d] transition hover:-translate-y-0.5 hover:border-white/[0.14] hover:shadow-2xl">
