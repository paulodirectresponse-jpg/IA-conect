...
export const SpaceToolbar:React.FC<Props>=({name,onNameChange,onBack,saving,saved,running,zoom,onZoomOut,onZoomIn,onFit,onAdd,onRunAll})=><header className="flex h-14 shrink-0 items-center gap-2 border-b border-white/[0.06] bg-[#071019]/95 px-3 sm:px-4 backdrop-blur-xl">
 ...
 <button onClick={onRunAll} disabled={running} className="inline-flex h-9 items-center gap-1.5 rounded-xl bg-cyan-300 px-3.5 text-[9px] font-black text-[#041019] shadow-[0_8px_30px_rgba(103,232,249,.12)] disabled:opacity-40">{running?<LoaderCircle className="h-3.5 w-3.5 animate-spin"/>:<Play className="h-3.5 w-3.5"/>}Executar tudo</button>
</header>;
