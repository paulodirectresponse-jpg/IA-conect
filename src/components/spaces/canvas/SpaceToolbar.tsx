...
interface Props{name:string;onNameChange:(value:string)=>void;onBack:()=>void;saving:boolean;saved:boolean;running:boolean;addOpen:boolean;zoom:number;onZoomOut:()=>void;onZoomIn:()=>void;onFit:()=>void;onAdd:()=>void;onRunAll:()=>void;}

export const SpaceToolbar:React.FC<Props>=({name,onNameChange,onBack,saving,saved,running,addOpen,zoom,onZoomOut,onZoomIn,onFit,onAdd,onRunAll})=><header className="flex min-h-14 shrink-0 items-center gap-2 overflow-x-auto border-b border-white/[0.06] bg-[#071019]/95 px-2.5 py-2 sm:h-14 sm:overflow-visible sm:px-4 sm:py-0 backdrop-blur-xl">
