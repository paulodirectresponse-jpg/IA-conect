...
export const SpaceNodeShell:React.FC<Props>=({node,selected,nodeRun,outputTypes,subtitle,children,onPointerDown,onInputPointerUp,onOutputPointerDown,onLabelChange,onDuplicate,onDelete,onRunFromHere,onInspect})=>{
 ...
 return <div data-space-node={node.node_id} data-selected={selected?'true':'false'} tabIndex={0} className={`group absolute overflow-hidden rounded-[16px] bg-[#071018] shadow-[0_14px_38px_rgba(0,0,0,.24)] transition-all duration-150 ${selected?'ring-1 ring-cyan-300/45 shadow-[0_18px_44px_rgba(34,211,238,.07)]':'ring-1 ring-white/[0.04] hover:ring-white/[0.10] hover:shadow-[0_18px_46px_rgba(0,0,0,.30)]'}`} style={{left:node.x,top:node.y,width:spaceNodeWidth(node),height:visualHeight||undefined,minHeight:visualHeight||SPACE_BASE_NODE_H}} onPointerDown={onPointerDown}>
  {node.kind==='TOOL'&&<button onPointerUp={onInputPointerUp} className={`absolute -left-1 top-1/2 z-30 h-3 w-3 -translate-y-1/2 rounded-full border-2 border-[#050a10] bg-zinc-600 transition-all duration-150 group-hover:bg-zinc-400 ${portVisibility}`} title="Entrada" aria-label="Entrada do node"/>}
