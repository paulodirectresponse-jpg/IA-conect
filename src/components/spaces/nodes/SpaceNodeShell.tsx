...
export const SpaceNodeShell:React.FC<Props>=({node,selected,nodeRun,outputTypes,subtitle,children,onPointerDown,onInputPointerUp,onOutputPointerDown,onLabelChange,onDuplicate,onDelete})=><div data-space-node={node.node_id} className={`absolute rounded-2xl border bg-[#0a141e]/97 shadow-[0_20px_70px_rgba(0,0,0,.35)] ${selected?'border-cyan-300/55 ring-1 ring-cyan-300/15':'border-white/[0.08]'}`} style={{left:node.x,top:node.y,width:spaceNodeWidth(node),minHeight:SPACE_BASE_NODE_H}} onPointerDown={onPointerDown}>
