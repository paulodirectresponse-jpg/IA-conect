...
export const SpaceNodeShell:React.FC<Props>=({node,selected,nodeRun,outputTypes,subtitle,children,onPointerDown,onInputPointerUp,onOutputPointerDown,onLabelChange,onDuplicate,onDelete,onRunFromHere,onInspect})=>{
 ...
 return <div data-space-node={node.node_id} className={`absolute overflow-hidden rounded-[18px] bg-[#071018] shadow-[0_18px_46px_rgba(0,0,0,.28)] transition-shadow ${selected?'ring-1 ring-cyan-300/55 shadow-[0_20px_56px_rgba(34,211,238,.08)]':'ring-1 ring-white/[0.055]'}`} style={{left:node.x,top:node.y,width:spaceNodeWidth(node),minHeight:SPACE_BASE_NODE_H}} onPointerDown={onPointerDown}>
  {node.kind==='TOOL'&&<button onPointerUp={onInputPointerUp} className="absolute -left-1.5 top-1/2 z-30 h-3.5 w-3.5 -translate-y-1/2 rounded-full border-2 border-[#050a10] bg-zinc-500" title="Entrada" aria-label="Entrada do node"/>}
  <button onPointerDown={onOutputPointerDown} className="absolute -right-1.5 top-1/2 z-30 h-3.5 w-3.5 -translate-y-1/2 rounded-full border-2 border-[#050a10] bg-cyan-300 shadow-[0_0_10px_rgba(103,232,249,.35)]" title="Arraste para conectar" aria-label="Saída do node; arraste para conectar"/>
  {generator?<div className="pointer-events-none absolute inset-x-0 top-0 z-20 flex items-center gap-2 bg-gradient-to-b from-black/65 to-transparent px-2.5 pb-7 pt-2">
  ...
  {children}
  {!generator&&<div className="flex items-center justify-between px-2 py-1.5"><span className="text-[6px] text-zinc-700">{node.kind==='TOOL'?'Saída: '+outputTypes.join(', '):node.media_type}</span>{actionButtons}</div>}
 </div>;
};
