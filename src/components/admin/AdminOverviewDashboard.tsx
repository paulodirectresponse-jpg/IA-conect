...
export const AdminOverviewDashboard:React.FC=()=>{
 ...
 const commercialCheck=useMemo(()=>health?.checks?.find(check=>check.key==='commercial')||null,[health]);
 ...
 return <div className="space-y-5">
  ...
  <section className="ia-admin-panel"><div className="flex items-center gap-2"><Activity className="w-4 h-4 text-sky-300"/><div><h2>Leitura executiva</h2><p>O que exige ação sem precisar abrir as outras abas.</p></div></div><div className="mt-4 grid sm:grid-cols-2 xl:grid-cols-4 gap-2 text-[10px]">{providerSummary.low>0&&<div className="rounded-xl border border-amber-300/15 bg-amber-300/[0.05] p-3 text-amber-200">{providerSummary.low} provider(s) com saldo baixo.</div>}{providerSummary.unavailable>0&&<div className="rounded-xl border border-rose-300/15 bg-rose-300/[0.05] p-3 text-rose-200">{providerSummary.unavailable} provider(s) indisponível(is) ou sem configuração.</div>}{healthSummary.degraded>0&&<div className="rounded-xl border border-amber-300/15 bg-amber-300/[0.05] p-3 text-amber-200">{healthSummary.degraded} check(s) degradados.</div>}{healthSummary.error>0&&<div className="rounded-xl border border-rose-300/15 bg-rose-300/[0.05] p-3 text-rose-200">{healthSummary.error} check(s) em erro.</div>}{commercialCheck&&<div className={`rounded-xl border p-3 ${commercialCheck.status==='OK'?'border-emerald-300/15 bg-emerald-300/[0.05] text-emerald-200':'border-amber-300/15 bg-amber-300/[0.05] text-amber-200'}`}><div className="font-bold">Monetização · {commercialCheck.status}</div><div className="mt-1 text-[9px] opacity-80">{commercialCheck.detail}</div></div>}{!loading&&alertCount===0&&commercialCheck?.status==='OK'&&<div className="rounded-xl border border-emerald-300/15 bg-emerald-300/[0.05] p-3 text-emerald-200">Nenhum alerta operacional relevante no momento.</div>}</div></section>
 </div>;
};
