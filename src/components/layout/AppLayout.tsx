...
export const AppLayout: React.FC<AppLayoutProps> = ({ currentView, onNavigate, betaEnabled = false, children }) => {
  ...
  const isCreateView = currentView === 'create-video' || currentView === 'create-image' || currentView === 'create-voice' || currentView === 'create-music' || currentView === 'create-3d' || currentView === 'edit-image' || currentView === 'edit-video' || currentView === 'spaces' || currentView === 'ai';
  ...
  useEffect(() => { markViewRendered(currentView); }, [currentView]);
  ...
  useEffect(() => {
    ...
    const onKeyDown = (event: KeyboardEvent) => { if (event.key === 'Escape' && sidebarOpen) setSidebarOpen(false); };
    syncLock(); media.addEventListener('change', syncLock); document.addEventListener('keydown', onKeyDown);
    return () => { media.removeEventListener('change', syncLock); document.removeEventListener('keydown', onKeyDown); document.documentElement.classList.remove('ia-mobile-nav-lock'); };
  }, [sidebarOpen]);
