...
import { BookOpen, Home, Images, ShieldCheck, Sparkles, X } from 'lucide-react';
    ...
    <>
      ...
      <aside className={`fixed inset-y-0 left-0 z-50 w-[204px] border-r border-white/[0.055] bg-[#080a0e] flex flex-col transition-transform duration-200 lg:static lg:translate-x-0 ${isOpen?'translate-x-0':'-translate-x-full'}`}>
        ...
        <div className="flex-1 overflow-y-auto px-2.5 py-4">
          <nav className="space-y-1">{nav.map(({id,label,icon:Icon})=>{const active=currentView===id;return <button key={id} onClick={()=>handleItemClick(id)} className={`group relative flex items-center gap-3 w-full h-10 px-3 rounded-xl text-[11px] font-semibold transition-all ${active?'bg-white/[0.075] text-white border border-white/[0.07]':'text-zinc-500 hover:text-zinc-200 hover:bg-white/[0.035]'}`}>{active&&<span className="absolute left-0 top-2 bottom-2 w-[2px] rounded-full bg-gradient-to-b from-violet-400 to-cyan-300"/>}<Icon className={`w-4 h-4 ${active?'text-violet-300':'text-zinc-600 group-hover:text-zinc-400'}`}/><span>{label}</span></button>;})}</nav>
