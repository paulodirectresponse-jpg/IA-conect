...
import { Bell, ChevronDown, CreditCard, LogOut, Menu, Plus, Settings, Shield, UserRound, Wallet as WalletIcon } from 'lucide-react';
  ...
  return (
    <header className="sticky top-0 z-30 flex items-center justify-between h-[70px] px-4 sm:px-6 bg-[#0b0e13]/95 backdrop-blur-xl border-b border-white/[0.06]">
      ...
      <div className="flex items-center gap-2 ml-auto">
        ...
        <button onClick={() => onNavigate('create')} className="hidden sm:flex h-9 items-center gap-1.5 px-3.5 rounded-xl bg-gradient-to-r from-violet-500 to-cyan-400 text-[11px] font-bold text-white shadow-[0_8px_24px_rgba(124,58,237,.18)] hover:brightness-110 transition-all"><Plus className="w-3.5 h-3.5"/> Criar</button>
        ...
        <div className="relative" ref={menuRef}>
          <button onClick={() => setOpen((v) => !v)} className={`h-9 pl-1 pr-2 flex items-center gap-2 rounded-xl border transition-all ${open ? 'border-violet-400/30 bg-violet-400/10' : 'border-transparent hover:border-white/[0.08] hover:bg-white/[0.04]'}`}>
            <div className="w-7 h-7 rounded-full bg-gradient-to-br from-violet-500 to-cyan-400 p-[1px]"><div className="w-full h-full rounded-full bg-[#11151c] flex items-center justify-center text-[10px] font-bold text-white">{initial}</div></div>
          ...
          {open && (
            <div className="absolute right-0 mt-2 w-64 overflow-hidden rounded-2xl border border-white/[0.08] bg-[#11151c] shadow-2xl shadow-black/50">
              <div className="p-3.5 border-b border-white/[0.06]"><p className="text-[12px] font-semibold text-white truncate">{profile?.display_name || 'Minha conta'}</p><p className="mt-0.5 text-[10px] text-zinc-500 truncate">{profile?.email}</p></div>
              <div className="p-1.5">
              ...
              <div className="p-1.5 border-t border-white/[0.06]"><button onClick={logout} className="w-full flex items-center gap-2.5 px-3 py-2.5 rounded-xl text-[11px] text-red-300 hover:bg-red-500/[0.08]"><LogOut className="w-4 h-4"/> Sair</button></div>
            </div>
          )}
        </div>
      </div>
