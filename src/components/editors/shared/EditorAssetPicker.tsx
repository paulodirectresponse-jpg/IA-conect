...
export const EditorAssetPicker:React.FC<EditorAssetPickerProps>=({open,assets,selectedId,uploading,loading,onClose,onSelect,onUpload})=>{
 ...
 return <div className="fixed inset-0 z-[90] flex items-end sm:items-center justify-center bg-black/70 p-0 sm:p-5 backdrop-blur-sm" onMouseDown={event=>{if(event.currentTarget===event.target)onClose();}}>
  <section className="w-full sm:max-w-4xl max-h-[86vh] overflow-hidden rounded-t-2xl sm:rounded-2xl border border-white/[0.08] bg-[#09121c] shadow-2xl">
   <header className="flex items-center gap-3 border-b border-white/[0.07] p-4">
    ...
    <label className="inline-flex h-9 cursor-pointer items-center gap-2 rounded-xl border border-cyan-400/20 bg-cyan-400/[0.07] px-3 text-[10px] font-semibold text-cyan-200 hover:bg-cyan-400/[0.12]"><Upload className="h-3.5 w-3.5"/>{uploading?'Enviando…':'Upload'}<input className="hidden" type="file" accept="image/*" onChange={onUpload}/></label>
   ...
   <div className="max-h-[58vh] overflow-y-auto px-4 pb-4">
    {loading?<div className="grid min-h-[220px] place-items-center rounded-2xl border border-white/[0.05] bg-white/[0.015]"><div className="text-center"><span className="mx-auto block h-5 w-5 animate-spin rounded-full border-2 border-cyan-300/70 border-t-transparent"/><p className="mt-2 text-[10px] text-zinc-500">Carregando a mesma Biblioteca do IA Connect…</p></div></div>:filtered.length?<div className="grid grid-cols-2 gap-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5">{filtered.map(asset=>{const preview=asset.thumbnail_url||asset.public_url;return <button key={asset.asset_id} onClick={()=>onSelect(asset)} className={`group overflow-hidden rounded-xl border text-left transition ${selectedId===asset.asset_id?'border-cyan-400/50 bg-cyan-400/[0.06]':'border-white/[0.06] bg-white/[0.02] hover:border-white/[0.14]'}`}>
      <div className="aspect-square bg-black/30">{preview?<img src={preview} alt={asset.name||'Imagem'} loading="lazy" decoding="async" referrerPolicy="no-referrer" className="h-full w-full object-cover"/>:<div className="grid h-full place-items-center"><ImageIcon className="h-6 w-6 text-zinc-700"/></div>}</div>
      <div className="truncate px-2.5 py-2 text-[9px] text-zinc-400 group-hover:text-zinc-200">{asset.name||'Imagem sem nome'}</div>
     </button>})}</div>:<div className="grid min-h-[220px] place-items-center rounded-2xl border border-dashed border-white/[0.07] text-center"><div><ImageIcon className="mx-auto h-8 w-8 text-zinc-700"/><p className="mt-2 text-[10px] text-zinc-500">Nenhuma imagem encontrada na Biblioteca Global.</p></div></div>}
   </div>
  </section>
 </div>;
};
