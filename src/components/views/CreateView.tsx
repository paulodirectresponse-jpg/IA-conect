...
export const CreateView:React.FC<Props>=({initialAsset,onEditImage})=>{
 ...
 const autoResolvedModel=useMemo(()=>[...autoCompatibleModels].sort((a,b)=>(quotedPricesByModelId[a.model_id]??Number.MAX_SAFE_INTEGER)-(quotedPricesByModelId[b.model_id]??Number.MAX_SAFE_INTEGER)||a.name.localeCompare(b.name))[0]||null,[autoCompatibleModels,quotedPricesByModelId]);
 ...
 const quickUpload=useCallback(async(target:'INITIAL'|'END'|'GENERAL',files:File[])=>{const usable=files.filter(file=>target==='GENERAL'||file.type.startsWith('image/'));if(!usable.length)return;setUploadBusy(true);try{for(const file of usable){const asset=await assetService.uploadAsset({file,category:file.type.startsWith('video/')?'MOTION':file.type.startsWith('audio/')?'AUDIO_REFERENCE':'GENERIC'});setAvailableAssets(prev=>[asset,...prev.filter(x=>x.asset_id!==asset.asset_id)]);if(target==='INITIAL'){setInitialImage(asset);setReferences([]);break}if(target==='END'){setEndImage(asset);setReferences([]);break}addGeneralAsset(asset)}}finally{setUploadBusy(false)}},[]);
