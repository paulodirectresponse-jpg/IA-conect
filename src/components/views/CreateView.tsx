...
function removeMentions(text:string,refs:WorkspaceReference[]){let next=text;for(const ref of refs){const escaped=ref.alias_snapshot.replace(/[|\\{}()[\]^$+*?.-]/g,'\\function ratioFromAsset(asset:Asset){');next=next.replace(new RegExp('@'+escaped+'(?=$|\\s|[.,!?;:])','gi'),'').replace(/ {2,}/g,' ')}return next.trim();}
...
export const CreateView:React.FC<Props>=({initialAsset,onEditImage})=>{
 ...
 const applyModelPlan=async(model:ModelRegistryItem)=>{
  ...
  const kept=new Set(plan.references.map(ref=>ref.asset_id)),removed=references.filter(ref=>!kept.has(ref.asset_id));
  setSelectionMode('MANUAL');setManualModelId(model.model_id);setResolution(plan.resolution);setDurationSeconds(plan.durationSeconds);setAspectRatio(plan.aspectRatio);setInitialImage(plan.initialImage);setEndImage(plan.endImage);setReferences(plan.references);if(removed.length)setPrompt(current=>removeMentions(current,removed));setAudioEnabled(plan.audioEnabled);setSeed(plan.seed);setMotionStrength(plan.motionStrength);setAdaptationNotice(plan.changes.map(item=>item.message).join(' '));setGenerationError('');
 ...
 const handleAssetPicked=(asset:Asset)=>{if(pickerTargetSlot==='INITIAL'){if(references.length)setPrompt(current=>removeMentions(current,references));setInitialImage(asset);setReferences([]);const ratio=ratioFromAsset(asset);if(ratio)setAspectRatio(ratio);setAdaptationNotice('Modo de quadro inicial ativado. O formato foi alinhado à imagem.')}else if(pickerTargetSlot==='END'){if(references.length)setPrompt(current=>removeMentions(current,references));setEndImage(asset);setReferences([]);setAdaptationNotice('Quadro final ativado.')}else{setInitialImage(null);setEndImage(null);addGeneralAsset(asset);setAdaptationNotice('Modo de referências ativado. Os frames ficaram fora desta geração.')}};
 ...
 const quickUpload=useCallback(async(target:'INITIAL'|'END'|'GENERAL',files:File[])=>{const usable=files.filter(file=>target==='GENERAL'||file.type.startsWith('image/'));if(!usable.length)return;setUploadBusy(true);try{for(const file of usable){const asset=await assetService.uploadAsset({file,category:file.type.startsWith('video/')?'MOTION':file.type.startsWith('audio/')?'AUDIO_REFERENCE':'GENERIC'});setAvailableAssets(prev=>[asset,...prev.filter(x=>x.asset_id!==asset.asset_id)]);if(target==='INITIAL'){if(references.length)setPrompt(current=>removeMentions(current,references));setInitialImage(asset);setReferences([]);const ratio=ratioFromAsset(asset);if(ratio)setAspectRatio(ratio);break}if(target==='END'){if(references.length)setPrompt(current=>removeMentions(current,references));setEndImage(asset);setReferences([]);break}setInitialImage(null);setEndImage(null);addGeneralAsset(asset)}}finally{setUploadBusy(false)}},[]);
