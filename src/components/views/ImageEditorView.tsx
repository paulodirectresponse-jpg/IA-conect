...
export const ImageEditorView:React.FC=()=>{
 ...
 useEffect(()=>{if(!eligible.some(model=>model.model_id===modelId)){setModelId(eligible.find(model=>model.model_id==='AUTO')?.model_id||eligible[0]?.model_id||'');}},[eligible,modelId]);
 const model=eligible.find(item=>item.model_id===modelId)||eligible.find(item=>item.model_id==='AUTO')||eligible[0]||null;
 ...
 const tools=<div className="flex gap-2 overflow-x-auto xl:flex-col xl:overflow-visible">{TOOLS.map(item=>{const Icon=item.icon;const available=models.some(model=>model.capabilities?.some((cap:any)=>cap.id===item.id));const selected=tool===item.id;return <button key={item.id} disabled={!available} onClick={()=>available&&setTool(item.id)} title={available?item.label:`${item.label} indisponível`} className={`min-w-[72px] rounded-xl border px-2 py-2.5 text-center transition xl:min-w-0 ${selected?'border-cyan-400/35 bg-cyan-400/[0.09] text-cyan-100 shadow-[inset_0_0_20px_rgba(34,211,238,.035)]':available?'border-white/[0.06] bg-white/[0.018] text-zinc-500 hover:border-white/[0.12] hover:text-zinc-300':'cursor-not-allowed border-white/[0.04] text-zinc-800'}`}><Icon className={`mx-auto h-4 w-4 ${selected?'text-cyan-300':''}`}/><span className="mt-1.5 block text-[8px] font-semibold leading-tight">{item.shortLabel}</span></button>;})}</div>;
 ...
 const inspector=<div className="flex min-h-full flex-col p-4">
  ...
  <div className="space-y-4 py-4">
   {eligible.length>1&&<label className="block"><span className="text-[9px] font-medium text-zinc-400">IA / modelo</span><select value={model?.model_id||''} onChange={e=>{setModelId(e.target.value);invalidate();}} className="mt-1.5 h-10 w-full rounded-xl border border-white/[0.07] bg-white/[0.025] px-3 text-[10px] text-zinc-300 outline-none focus:border-cyan-400/25">{eligible.map(item=><option key={item.model_id} value={item.model_id}>{item.model_id==='AUTO'?'AUTO · melhor rota disponível':item.name}</option>)}</select></label>}
 ...
 return <>
  ...
  <EditorAssetPicker open={pickerOpen} assets={images} selectedId={sourceId} busy={busy==='upload'} onClose={()=>setPickerOpen(false)} onSelect={selectAsset} onUpload={upload}/>
 </>;
};
