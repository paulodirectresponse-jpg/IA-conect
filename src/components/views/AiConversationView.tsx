...
import{AiConversationActionCard}from'../ai/AiConversationActionCard.js';
...
export const AiConversationView:React.FC=()=>{
 ...
 const confirmAction=async(action:AiConversationAction)=>{if(!current||actionBusy)return;setActionBusy(action.action_id);setError('');try{const next=await aiConversationClient.confirmAction(current.conversation_id,action.action_id);replaceAction(next);if(next.status==='AWAITING_CONFIRMATION')setError('A cotação expirou e foi atualizada. Confira o novo custo antes de confirmar novamente.');}catch(e){setError(errText(e));try{replaceAction(await aiConversationClient.refreshAction(current.conversation_id,action.action_id));}catch{}}finally{setActionBusy(null);}};
 const regenerateResult=async(action:AiConversationAction,assetId:string)=>{if(!current||actionBusy)return;setActionBusy(action.action_id);setError('');try{const next=await aiConversationClient.regenerateResult(current.conversation_id,action.action_id,assetId);setActions(rows=>[...rows.filter(row=>row.action_id!==next.action_id),next]);}catch(e){setError(errText(e));}finally{setActionBusy(null);}};
 const retryAction=async(action:AiConversationAction)=>{if(!current||actionBusy)return;setActionBusy(action.action_id);setError('');try{const next=await aiConversationClient.retryAction(current.conversation_id,action.action_id);setActions(rows=>[...rows.filter(row=>row.action_id!==next.action_id),next]);}catch(e){setError(errText(e));}finally{setActionBusy(null);}};
 const cancelAction=async(action:AiConversationAction)=>{if(!current||actionBusy)return;setActionBusy(action.action_id);setError('');try{replaceAction(await aiConversationClient.cancelAction(current.conversation_id,action.action_id));}catch(e){setError(errText(e));}finally{setActionBusy(null);}};
