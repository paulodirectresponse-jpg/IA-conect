...
export const SpacesView:React.FC=()=>{
 ...
 const create=async()=>{setCreating(true);setError('');try{const flow=await spacesClient.create({name:'Space sem título',description:'Workspace visual de criação.',project_id:null,graph:{nodes:[],edges:[]}});setFlows(rows=>[flow,...rows.filter(row=>row.flow_id!==flow.flow_id)]);setHomeItems(rows=>[{flow,cover_asset:null,recent_assets:[]},...rows.filter(row=>row.flow.flow_id!==flow.flow_id)]);setCurrent(flow);}catch(e){setError(errorText(e));}finally{setCreating(false);}};
 ...
 return <SpacesHome flows={flows} homeItems={homeItems} loading={loading} creating={creating} deletingId={deletingId} error={error} onCreate={()=>void create()} onOpen={id=>void open(id)} onDelete={flow=>void remove(flow)}/>;
};
