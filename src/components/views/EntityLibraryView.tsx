...
export const EntityLibraryView: React.FC<{ kind: CreativeEntityKind; embedded?: boolean; projectId?: string | null }> = ({ kind, embedded = false, projectId }) => {
  ...
  const save = async () => {
    ...
    try {
      await creativeEntityService.save({ kind, name, description, project_id: kind === 'PROJECT' ? null : (projectId || null), cover_asset_id:selectedCover?.asset_id||null, cover_url:selectedCover?.thumbnail_url||selectedCover?.public_url||null, asset_ids:selectedCover?[selectedCover.asset_id]:[] });
  ...
  return (
    <div className="space-y-4 pb-8">
      <section className="flex flex-col md:flex-row md:items-end justify-between gap-4">
        <div>
          {!embedded && <div className="flex items-center gap-2 text-[10px] font-bold uppercase tracking-[0.18em] text-cyan-300"><Icon className="w-4 h-4" /> Biblioteca criativa</div>}
