...
export const EntityLibraryView: React.FC<{ kind: CreativeEntityKind }> = ({ kind }) => {
  ...
  useEffect(() => {
    ...
    assetService.listAssets().then(setAssets).catch(() => setAssets([]));
  }, [kind]);
  ...
  const save = async () => {
    ...
    setSaving(true);
    try {
      await creativeEntityService.save({
        kind,
        name,
        description,
        cover_asset_id: selectedCover?.asset_id || null,
        cover_url: selectedCover?.thumbnail_url || selectedCover?.public_url || null,
        asset_ids: selectedCover ? [selectedCover.asset_id] : [],
      });
      setOpen(false); setName(''); setDescription(''); setCoverId('');
      await refresh();
  ...
  return (
    <div className="space-y-5 pb-8">
      <section className="flex flex-col md:flex-row md:items-end justify-between gap-4">
        <div>
          <div className="flex items-center gap-2 text-[10px] font-bold uppercase tracking-[0.18em] text-violet-400"><Icon className="w-4 h-4" /> Biblioteca criativa</div>
          <h1 className="mt-2 text-2xl font-black tracking-[-0.035em] text-white">{info.title}</h1>
          <p className="mt-1 text-xs text-zinc-500 max-w-2xl">{info.description}</p>
        </div>
        <button onClick={() => setOpen(true)} className="h-10 px-4 rounded-xl bg-gradient-to-r from-violet-500 to-cyan-400 text-[11px] font-bold text-white flex items-center gap-2 shadow-[0_10px_30px_rgba(124,58,237,.18)] hover:brightness-110"><Plus className="w-4 h-4" /> Novo {info.singular.toLowerCase()}</button>
      </section>
