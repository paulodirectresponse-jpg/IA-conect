...
export const VoiceCreateView:React.FC=()=>{
 ...
 const[models,setModels]=useState<VoiceModel[]>([]),[selectedModelId,setSelectedModelId]=useState('AUTO'),[text,setText]=useState(''),[voice,setVoice]=useState('calm-female'),[language,setLanguage]=useState('auto'),[format,setFormat]=useState('mp3'),[openCard,setOpenCard]=useState<OpenCard>(null),[mobileActiveCount,setMobileActiveCount]=useState(0);
 ...
 const load=useCallback(async()=>{setBusy(current=>current||'load');setError('');try{const available=(await voiceGenerationClient.catalog()).filter(model=>model.capabilities.some(capability=>capability.id==='text-to-speech'));setModels(available);const fallback=available.find(model=>model.model_id==='AUTO'&&model.providers?.length)?.model_id||available.find(model=>model.model_id!=='AUTO'&&model.providers?.length)?.model_id||available.find(model=>model.model_id!=='AUTO')?.model_id||'';setSelectedModelId(current=>current==='AUTO'?fallback:(available.some(model=>model.model_id===current)?current:fallback));}catch(err){setError(message(err));}finally{setBusy(current=>current==='load'?'':current)}},[]);
 ...
 const routeReady=selectedModelId==='AUTO'?Boolean(models.find(model=>model.model_id==='AUTO')?.providers?.length):Boolean(selectedModel?.providers?.length);
 ...
 const creator=<GeneratorPanel ariaLabel="Gerador de voz"><GeneratorScroll>
  <StableGeneratorModelPicker models={models.filter(model=>model.model_id!=='AUTO'||Boolean(model.providers?.length)).map(model=>({model_id:model.model_id,name:model.name,description:'Modelo de voz'}))} selectedModelId={selectedModelId} loading={busy==='load'} onSelect={modelId=>{setSelectedModelId(modelId);invalidate()}}/>
